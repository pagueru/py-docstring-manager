"""
Funcionalidade para manipular docstrings em scripts Python de forma programática, utilizando arquivos YAML como fonte de mapeamento. 
...
DocMap = Dict[str, Dict[str, str]]
"""Tipagem para o mapeamento de docstrings."""
...
def remove_docstrings(
    ...
    """
    Remove docstrings do script Python com base em um arquivo YAML ou em uma lista/nome de funções/classes.

    Args:
        script_path (str): O caminho do script Python.
        yaml_path (str, optional): O caminho do arquivo YAML contendo as docstrings a serem removidas.
            Se fornecido, utiliza o mapeamento YAML.
        target_names (list | str, optional): Lista ou nome único de funções/classes cujas docstrings
            devem ser removidas. Ignorado se o `yaml_path` for fornecido.
            Se ambos `yaml_path` e `target_names` forem `None`, remove todas as docstrings.
    """
    ...
    def remove_action(
        ...
        should_remove = (
            (doc_map is None and (target_names is None or name in target_names)) or
            (doc_map is not None and docstring is not None)
        )
...
def add_docstrings_from_yaml(
    ...
    def add_action(
        ...
        if docstring:
            ...
            formatted_docstring = f'{indentation}"""\n{docstring.strip()}\n{indentation}"""'
